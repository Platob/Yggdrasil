...
The cast registry, schema layer, and engine bridges are all tuned for the hot path — type checks, equality, hash, projection, and same-shape merges live in the nanosecond range so per-batch overhead stays negligible. Numbers below come from `python/benchmarks/run_all.py` and are updated automatically on release.

<!-- BENCH_RESULTS_START -->
<!-- Regenerated by python/benchmarks/update_readme.py — do not edit by hand. -->

Whole-table cast latency on representative analytics shapes — the **convenient** schema mixes int / decimal / string / timestamp / bool / date, and **nested** stresses list / map / struct round-trips. Numbers are wall-clock per call, 10k rows, taken from `bench_cast_data.py`.

| Operation | Per call (best) |
|---|---|
| Arrow → Arrow, convenient types (int/decimal/string/timestamp/bool/date), 10k rows | ~813.5 µs |
| Arrow → Arrow, nested types (list / map / struct), 10k rows | ~310.8 µs |
| Polars → Polars, convenient types, 10k rows | ~1.1 ms |
| Polars → Polars, nested types, 10k rows | ~935.4 µs |
| pandas → pandas, convenient types, 10k rows | ~9.2 ms |
...
Per-column kernel timings (Arrow / Polars / pandas, MATCH / WIDEN / NARROW), schema-merge, equality, and `CastOptions` overhead all live in [`python/benchmarks/`](python/benchmarks/) and are picked up automatically by `run_all.py`.
