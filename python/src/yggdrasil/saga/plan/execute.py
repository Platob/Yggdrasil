...
def _exec_select(node: SelectNode, tables: dict[str, "Tabular"]) -> "Tabular":
    from yggdrasil.arrow.tabular import ArrowTabular
    from yggdrasil.saga.expr.nodes import Alias, Column, FunctionCall, Literal, Star
    ...
    if spark_frame is not None and (node.group_by is not None or node.set_ops):
        try:
            from yggdrasil.saga.plan.sql_emitter import emit_sql
...
def _exec_group_by(result: "Tabular", node: SelectNode) -> "tuple[Tabular, dict | None]":
    from yggdrasil.arrow.tabular import ArrowTabular
    from yggdrasil.saga.expr.nodes import Alias, Column, FunctionCall, Star
    import pyarrow.compute as pc
...
def _rewrite_having(having: Any, result: "Tabular", agg_alias_map: dict) -> Any:
    from yggdrasil.saga.expr.nodes import Column, Comparison, FunctionCall, Logical, Not, Star
    from yggdrasil.saga.expr import walk
...
def _apply_lateral_views(result: "Tabular", lateral_views: list) -> "Tabular":
    ...
    """
    from yggdrasil.arrow.tabular import ArrowTabular
    from yggdrasil.saga.expr.nodes import Column, FunctionCall
