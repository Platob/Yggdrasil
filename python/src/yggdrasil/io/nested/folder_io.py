...
class Folder(Tabular[FolderOptions]):
    ...
    def _has_tabular_children(self) -> bool:
        ...
        Short-circuits on the first hit, so it costs one ``iterdir``
        step instead of walking the whole directory. Distinct from
        the class-level :attr:`Tabular.has_children` flag (which
        reports the *kind* of Tabular, not whether this particular
        directory currently holds anything).
        """
