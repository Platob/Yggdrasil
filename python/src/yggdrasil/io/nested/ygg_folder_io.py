...
class YGGFolderIO(FolderIO):
    ...
    def _write_compact_bucket(
        ...
        *,
        ...
        for source in ordered_sources:
            try:
                source_io = Tabular.from_path(source)
            except Exception:
                return
            with source_io:
                try:
        ...
        try:
            target_io = Tabular.from_path(staging, media_type=media_type)
            with target_io:
    ...
    def spark_connector(self) -> "Any":
        ...
        from yggdrasil.io.nested.ygg_folder_spark import YGGFolderSparkConnector
