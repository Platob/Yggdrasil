    ...
    # ── String: keyword mapping ───────────────────────────────────────────────
        ...
        if options.safe:
            ...
            result = (
                ...
                .when(true_expr).then(pl.lit(True))
                .when(false_expr).then(pl.lit(False))
                .otherwise(pl.lit(None, dtype=pl.Boolean))
            )
        else:
            result = (
                pl.when(true_expr).then(pl.lit(True))
                .when(false_expr).then(pl.lit(False))
                .otherwise(pl.lit(None, dtype=pl.Boolean))
            )
...
def cast_polars_array_to_list(
    ...
    def _child_options(src_arrow_inner: pa.Field, tgt_arrow_inner: pa.Field) -> "CastOptions":
        return options.copy(
            source_field=src_arrow_inner,
            target_field=tgt_arrow_inner,
        )
    ...
    if (
        pa.types.is_list(saf.type) or pa.types.is_large_list(saf.type)
        or pa.types.is_list_view(saf.type) or pa.types.is_large_list_view(saf.type)
    ):
        ...
        result = (
            base.list.eval(inner_expr, parallel=True)
                .cast(tpf.dtype, strict=options.safe)
                .alias(series_name)
        )
        return _eval(result)
    ...
    if pa.types.is_fixed_size_list(saf.type):
        ...
        result = (
            as_list.list.eval(inner_expr, parallel=True)
            .cast(tpf.dtype, strict=options.safe)
            .alias(series_name)
        )
        return _eval(result)
...
def cast_polars_array(
    ...
    if spdt.__class__ is pl.Null:
        if is_expr:
            if tpf.nullable:
                out: Union[pl.Series, pl.Expr] = array.cast(tpdt)
            else:
        ...
        if to_expr:
            ...
            if tpf.nullable:
                ...
                dv = default_arrow_scalar(options.target_field.type, nullable=tpf.nullable).as_py()
                out = pl.lit(dv, dtype=tpdt).alias(tpf.name)
            return out
    ...
    # ── Boolean target ────────────────────────────────────────────────────────
        ...
        array = cast_polars_array_to_bool(array, options, to_expr=to_expr)
...
def cast_polars_dataframe(
    ...
    for col_pos, tgt_arrow_field in enumerate(target_schema):
        ...
        if src_idx is None:
            ...
            dv = default_arrow_scalar(
                tgt_arrow_field.type,
                nullable=tgt_arrow_field.nullable,
            ).as_py()
...
def any_polars_to_arrow_field(
    ...
    if isinstance(obj, pl.Schema):
        ...
        arrow_type = pa.struct([
            polars_field_to_arrow_field(pl.Field(name=k, dtype=t))
