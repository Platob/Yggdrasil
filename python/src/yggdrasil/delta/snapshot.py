...
- :attr:`txn`             — last committed application txn map.
...
import dataclasses
from typing import TYPE_CHECKING, Dict, Iterable, Iterator, List, Mapping, Optional, Tuple
...
class Snapshot:
    ...
    def prune_files(
        ...
        *,
        ...
        ``IN``-set semantics — same shape :class:`YGGFolderIO` uses
...
def _to_str_set(values: "Iterable") -> "frozenset":
    ...
    """
    out: list[str] = []
    for v in (values if not isinstance(values, (str, bytes, int, float, bool)) else (values,)):
        if v is None:
