"""Pure-Python deletion-vector decoder.
...
mask out those rows before yielding the batch upstream.

Three storage shapes
--------------------

- ``"i"`` (inline) — the bitmap bytes are base85-z85 encoded straight
  into the descriptor's ``pathOrInlineDv``. No sidecar fetch.
...
This module ships a minimal "fast enough for any reasonable file
size" decoder for both. We don't need the write side here — that
lives on the writer once we start emitting DVs.
"""
...
from yggdrasil.delta.protocol import DeletionVectorDescriptor

if TYPE_CHECKING:
    from yggdrasil.io.path import Path


__all__ = ["DeletionVector", "DeletionVectorDescriptor", "decode_deletion_vector"]


# ---------------------------------------------------------------------------
# Z85 / base85 inline decoding
# ---------------------------------------------------------------------------
...
# :func:`base64.b85decode`. The table is 1:1 — same character set,
# same padding rules — only the symbol-to-value mapping is permuted.
...
def _read_portable_roaring(buf: memoryview, pos: int) -> "tuple[Set[int], int]":
    ...
    for idx, (key, card) in enumerate(key_card):
        high = key << 16
        is_run = bool(run_flag_bytes[idx >> 3] & (1 << (idx & 7))) if has_run_flag else False
...
def mask_batch_with_dv(
    ...
    *,
    ...
    Imported lazily to keep ``yggdrasil.delta.deletion_vector`` cheap.
    """
    ...
    keep = [
        i for i in range(n)
        if (base_offset + i) not in deleted
    ]
    if len(keep) == n:
        return batch
    if not keep:
