...
- ``00000000000000000010.checkpoint.<uuid>.json`` /
  ``00000000000000000010.checkpoint.<uuid>.parquet`` plus
  sidecar parquet files under ``_delta_log/_sidecars/`` — V2 checkpoints
  (UUID-named, optionally split into per-action-class sidecars).
...
This module owns step 1 and step 2; :class:`yggdrasil.delta.snapshot.Snapshot`
owns step 3 + reduction.
"""
...
import json
import re
from typing import TYPE_CHECKING, Any, Iterable, Iterator, List, Mapping, Optional, Tuple

from yggdrasil.delta.protocol import DeltaAction, parse_action
...
#: ``00000000000000000007.json`` — 20-zero-padded version + suffix.
_VERSION_RE = re.compile(
    r"^(?P<version>\d{20})"
    r"(?:\.checkpoint(?:\.(?P<uuid>[0-9A-Fa-f-]+))?(?:\.(?P<part>[0-9]+))?(?:\.parquet|\.json)|\.json)$"
)

#: Used by :func:`format_checkpoint_v1_name` and friends.
_VERSION_FMT = "{:020d}"
...
def version_from_log_name(name: str) -> "Optional[int]":
    """Extract the integer version embedded in a log filename.

    Returns ``None`` for files we don't recognise (``_last_checkpoint``,
    crc files, the ``_sidecars`` directory).
    """
...
class LogSegment:
    ...
    #: Optional V2 manifest — when present, the format is
    #: ``("v2", <manifest_path>)``; ``("v1", None)`` for V1 (single
    #: parquet) or ``("none", None)`` when no checkpoint exists.
...
class DeltaLog:
    ...
    schema-only follow-up don't re-list the directory; pass
    ``listing_ttl=0.0`` to disable caching when the log is being
    written under your feet.
    """
    ...
    def __init__(self, table_root: "Path") -> None:
        ...
        self._last_checkpoint: "Optional[Mapping[str, Any]]" = None
    ...
    def read_last_checkpoint(self) -> "Optional[Mapping[str, Any]]":
    ...
    def segment(self, version: "Optional[int]" = None) -> LogSegment:
        ...
        ck_version_hint = (
            int(last_ck["version"])
            if last_ck and "version" in last_ck
            else -1
        )
        ...
        ck_version, ck_files, ck_kind = self._resolve_checkpoint(
            listing, target, ck_version_hint, last_ck,
        )

        # 4. Commits strictly above ck_version, up to and including target.
        commits = tuple(
            self.log_path / name
            for v, name in all_commits
            if ck_version < v <= target
        )
    ...
    def _resolve_checkpoint(
        ...
        target: int,
        hint: int,
        last_ck: "Optional[Mapping[str, Any]]",
    ...
    def _files_for_checkpoint(
        ...
        if last_ck and last_ck.get("v2Checkpoint"):
            sidecars = last_ck["v2Checkpoint"].get("sidecars") or []
            if sidecars:
                return tuple(
    ...
    def _read_v2_sidecars(
        self, version: int, manifest_name: str,
    ) -> "Tuple[Path, ...]":
    ...
    def replay_raw(self, segment: LogSegment) -> "Iterator[Mapping[str, Any]]":
    ...
    def _iter_commit_raw(self, path: "Path") -> "Iterator[Mapping[str, Any]]":
        try:
    ...
    def _iter_checkpoint_raw(
        ...
        """
        # Lazy import — we want :mod:`yggdrasil.delta.log` importable
        # without pyarrow.parquet on the path (the import lives in
        # snapshot/io anyway).
