...
def _coerce_field(candidate: pa.Field | pa.DataType, default_name: str) -> pa.Field:
    """Return a :class:`pyarrow.Field` for *candidate*.

    This helper accepts either an existing :class:`~pyarrow.Field` or a
    :class:`~pyarrow.DataType` and normalises the value so downstream code can
    always rely on field semantics.
    """
...
class ArrowArrayCaster:
    ...
    def __post_init__(self) -> None:  # pragma: no cover - trivial normalisation
        object.__setattr__(self, "source_field", _coerce_field(self.source_field, "source"))
        object.__setattr__(self, "target_field", _coerce_field(self.target_field, "target"))
...
class ArrowCastRegistry:
    ...
    def register(self, caster: ArrowArrayCaster) -> None:
        ...
        source_field = _coerce_field(caster.source_field, "source")
        target_field = _coerce_field(caster.target_field, "target")

        if source_field is not caster.source_field or target_field is not caster.target_field:
            caster = ArrowArrayCaster(
                source_field=source_field,
                target_field=target_field,
                _cast_impl=caster._cast_impl,
            )

        self._casters[self._key(source_field, target_field)] = caster
    ...
    def get_or_build(
        ...
        source_field = _coerce_field(source_field, "source")
        target_field = _coerce_field(target_field, "target")
    ...
    def _key(
        ...
        return self._type_key(source_field), self._type_key(target_field)

    def _type_key(self, candidate: pa.Field | pa.DataType) -> str:
        if isinstance(candidate, pa.Field):
            dtype = candidate.type
        elif isinstance(candidate, pa.DataType):
            dtype = candidate
        else:
            raise TypeError(
                "Keys must be derived from pyarrow.Field or pyarrow.DataType instances"
            )
        return str(dtype)
