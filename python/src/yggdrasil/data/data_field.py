...
class Field(BaseMetadata, BaseChildrenFields):
    ...
    def alias(self) -> str:
        ...
        Stored in :attr:`metadata` under :data:`ALIAS_KEY`. Carries
        a secondary label for the field (e.g. a wire name vs. a
        friendly name) so downstream tooling can present either
    ...
    def position(self) -> int | None:
        ...
        Stored in :attr:`metadata` under :data:`POSITION_KEY` and
        surfaced into Databricks-side metadata by
        :mod:`yggdrasil.databricks.sql.types`. ``None`` (the default)
