...
def _render_callable_script(
    ...
    return (
        ...
        # ``_ygg_runtime`` carries the runtime helpers the workflow layer
        # leans on — secret() resolves a SecretRef against the current
        # workspace client, task_value() reads a value an upstream task
        # published via dbutils.jobs.taskValues, publish_return() wraps
        # the staged invocation so this task's return surfaces on the
        # task-values map. Imported unconditionally so every staged
...
def _render_callable_notebook(
    ...
    metadata_cell = (
        ...
        "from yggdrasil.dataclasses.safe_function import checkargs\n"
        # ``_ygg_runtime`` carries the workflow runtime helpers — see the
        # matching script renderer for the rationale; imported in every
        # staged notebook so SecretRef / TaskNode reprs evaluate cleanly.
        "from yggdrasil.databricks.workflow import runtime as _ygg_runtime\n"
        f"__yggdrasil_task__ = _yggdrasil_json.loads(r\"\"\"{meta_json}\"\"\")\n"
...
def _render_invocation_cell(
    ...
    return (
        ...
        f"    _ygg_result = _ygg_runtime.publish_return({invocation})\n"
        f"except Exception:\n"
...
def _render_invocation_script(
    ...
    # ``publish_return`` wraps the invocation so the staged function's
    # return value lands on ``dbutils.jobs.taskValues`` for downstream
    # tasks to read via ``task_value``. Silent no-op outside Databricks
    # so local ``python <script>`` re-runs still terminate normally.
    if not unbound:
        return (
            'if __name__ == "__main__":\n'
            f"    _ygg_runtime.publish_return({invocation})\n"
        )
    return (
