...
Native storage access (explicit, opt-in)
-----------------------------------------
...
:meth:`temporary_credentials`, and :meth:`aws` expose the volume's
UC-vended S3 storage so a caller that *deliberately* wants the
object store can reach it — the resulting :class:`S3Path` carries a
botocore :class:`RefreshableCredentials`-backed session that
re-invokes :meth:`temporary_credentials` on every near-expiry
refresh cycle.

This is **never** done implicitly. The :class:`Holder` byte
primitives (:meth:`_read_mv` / :meth:`_upload` / :meth:`_upload_stream`)
always go through the Files API off-cluster (or the kernel mount on a
Databricks runtime). Silently re-routing a volume read / write to the
backing bucket bypasses Unity Catalog governance (grants, audit,
lineage) and the UC-managed storage layout, so the path operates
exactly where the URL says it does — under ``/Volumes`` — and direct
object-store access stays an explicit choice the caller makes.
