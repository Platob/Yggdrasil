...
A serverless python-wheel task invokes the single ``ygg`` entry point
(``package_name="ygg"``, ``entry_point="ygg"``) with a leading ``run``
subcommand, the **target** of the decorated object, plus either a pickled
argument payload (the transparent ``__call__`` dispatch from a laptop) or
positional string parameters (a scheduled / triggered deploy)::
