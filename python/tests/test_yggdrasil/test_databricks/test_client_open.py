...
class TestDatabricksClientOpen:
    ...
    def test_string_path_routes_through_dbfs_path(self, client):
        # The string lands on ``dbfs_path`` (which builds a
        # :class:`DatabricksPath` bound to this client) and the
        # returned path's ``.open`` does the work.
        fake_path = MagicMock()
        with patch.object(
            DatabricksClient, "dbfs_path", return_value=fake_path,
        ) as dbfs_path:
            result = client.open("/Volumes/cat/sch/vol/x", "rb")
        dbfs_path.assert_called_once_with("/Volumes/cat/sch/vol/x")
    ...
    def test_existing_path_is_opened_verbatim(self, client):
        # Already a :class:`Path` — skip ``dbfs_path`` and call its
        # ``.open`` directly so callers can mix in S3/HTTP/local paths
        # without losing the workspace binding.
        path = MagicMock(spec=VolumePath)
        with patch.object(DatabricksClient, "dbfs_path") as dbfs_path:
            client.open(path, "wb")
        dbfs_path.assert_not_called()
    ...
    def test_default_mode_is_none(self, client):
        ...
        fake_path = MagicMock()
        with patch.object(DatabricksClient, "dbfs_path", return_value=fake_path):
