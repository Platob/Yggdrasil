...
atexit registration, error handling (InternalError, PermissionDenied,
...
class TestCommandExecutionConstruction(unittest.TestCase):
    ...
    def test_environ_dict_conversion(self):
        ctx = _make_ctx()
        # Non-mapping iterables raise ValueError
    ...
    def test_temporary_defaults_to_false(self):
        ...
        self.assertFalse(cmd.temporary)
...
class TestCommandExecutionState(unittest.TestCase):
    ...
    def test_running_true_when_running(self):
        ...
        # details property re-fetches from API when status is RUNNING;
        # seed the API mock so it still returns RUNNING
...
class TestCommandExecutionDetails(unittest.TestCase):
    ...
    def test_details_done_unregisters_atexit(self, mock_unreg):
        cmd = _make_cmd(command_id="cmd-d2", temporary=True)
        cmd._details = _finished("cmd-d2")
        _ = cmd.details
        mock_unreg.assert_called()

    @patch("atexit.unregister")
    def test_details_running_does_not_unregister_atexit(self, mock_unreg):
        cmd = _make_cmd(command_id="cmd-d3", temporary=True)
        cmd._details = _running("cmd-d3")
        # Accessing details when running → re-fetches from API
    ...
    def test_details_fetches_from_api_when_running(self):
        cmd = _make_cmd(command_id="cmd-d4", temporary=True)
...
class TestCommandExecutionStart(unittest.TestCase):
    ...
    def test_start_sets_command_id(self, _):
        ctx = _make_ctx(context_id="ctx-s1", temporary=True)
    ...
    @patch("atexit.register")
    def test_start_temporary_registers_atexit(self, mock_reg):
        ctx = _make_ctx(context_id="ctx-s2", temporary=True)
    ...
    @patch("atexit.register")
    def test_start_non_temporary_no_atexit(self, mock_reg):
        ctx = _make_ctx(context_id="ctx-s3", temporary=False)
        _ws(ctx.cluster).execute.return_value = _exec_response("cmd-s3")
    ...
    def test_start_no_reset_returns_self_when_running(self):
        ctx = _make_ctx(context_id="ctx-s4", temporary=True)
        cmd = _make_cmd(ctx=ctx, command_id="cmd-s4-existing", temporary=True)
    ...
    def test_start_asserts_command_present(self, _):
        ctx = _make_ctx(context_id="ctx-s5", temporary=True)
        cmd = _make_cmd(ctx=ctx, command="", temporary=True)
        cmd.command = None
    ...
    def test_start_reset_running_cancels_and_reregisters(self, mock_reg, mock_unreg):
        ctx = _make_ctx(context_id="ctx-sr1", temporary=True)
        ws = _ws(ctx.cluster)
        ws.execute.return_value = _exec_response("cmd-new-sr1")
        ws.cancel.return_value = MagicMock()

        cmd = _make_cmd(ctx=ctx, command_id="cmd-old-sr1", temporary=True)
    ...
    def test_start_reset_done_unregisters_before_reregistering(self, mock_reg, mock_unreg):
        """Bug guard: done-reset must not leave a duplicate atexit entry."""
        ctx = _make_ctx(context_id="ctx-sr2", temporary=True)
        _ws(ctx.cluster).execute.return_value = _exec_response("cmd-new-sr2")

        cmd = _make_cmd(ctx=ctx, command_id="cmd-done-sr2", temporary=True)
    ...
    @patch("atexit.unregister")
    @patch("atexit.register")
    def test_start_reset_done_registration_count_is_one(self, mock_reg, mock_unreg):
        """Verify the registry has exactly 1 entry after a done-reset."""
        registry: list = []

        ctx = _make_ctx(context_id="ctx-sr3", temporary=True)
        _ws(ctx.cluster).execute.return_value = _exec_response("cmd-new-sr3")

        cmd = _make_cmd(ctx=ctx, command_id="cmd-done-sr3", temporary=True)
        cmd._details = _finished("cmd-done-sr3")

        with patch("atexit.register", side_effect=lambda f: registry.append(f)), \
             patch("atexit.unregister", side_effect=lambda f: registry.__delitem__(
                 next(i for i, x in enumerate(registry) if x == f)
             ) if f in registry else None):
            registry.append(cmd._unsafe_cancel)   # simulate initial registration
            cmd.start(reset=True)
    ...
    def test_start_retries_on_internal_error(self, _):
        ctx = _make_ctx(context_id="ctx-se1", temporary=True)
        ws = _ws(ctx.cluster)
        # First execute raises InternalError; second succeeds (after reconnect)
        ws.execute.side_effect = [InternalError("boom"), _exec_response("cmd-se1-retry")]
        # connect(reset=True) is called internally; returns a new ctx with same cluster
    ...
    def test_start_retries_on_context_id_error(self, _):
        ctx = _make_ctx(context_id="ctx-se2", temporary=True)
        ws = _ws(ctx.cluster)
        ws.execute.side_effect = [
            RuntimeError("invalid context_id"),
            _exec_response("cmd-se2-retry"),
        ]
        ctx.cluster.client.workspace_client.return_value.command_execution.create.return_value = (
    ...
    def test_start_propagates_unrecognised_error(self, _):
        ctx = _make_ctx(context_id="ctx-se3", temporary=True)
...
class TestCommandExecutionCancel(unittest.TestCase):
    ...
    def test_cancel_calls_api_and_clears_command_id(self, _):
        ctx = _make_ctx(context_id="ctx-c1", temporary=True)
        _ws(ctx.cluster).cancel.return_value = MagicMock()

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c1", temporary=True)
        cmd.cancel()
    ...
    @patch("atexit.unregister")
    def test_cancel_temporary_unregisters_atexit(self, mock_unreg):
        ctx = _make_ctx(context_id="ctx-c2", temporary=True)
        _ws(ctx.cluster).cancel.return_value = MagicMock()

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c2", temporary=True)
        cmd.cancel()

        mock_unreg.assert_called_once()
        self.assertIs(mock_unreg.call_args[0][0].__self__, cmd)

    @patch("atexit.unregister")
    def test_cancel_non_temporary_no_atexit(self, mock_unreg):
        ctx = _make_ctx(context_id="ctx-c3", temporary=False)
        _ws(ctx.cluster).cancel.return_value = MagicMock()

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c3", temporary=False)
        cmd.cancel()
    ...
    def test_cancel_no_command_id_is_noop(self, mock_unreg):
        ctx = _make_ctx(context_id="ctx-c4", temporary=True)
    ...
    def test_cancel_no_context_id_is_noop(self, mock_unreg):
        """cancel() when context.context_id is None/'' must be a no-op."""
        ctx = _make_ctx(context_id="", temporary=True)
        cmd = _make_cmd(ctx=ctx, command_id="cmd-c5", temporary=True)
    ...
    def test_cancel_suppresses_errors_by_default(self, _):
        ctx = _make_ctx(context_id="ctx-c6", temporary=True)
        _ws(ctx.cluster).cancel.side_effect = RuntimeError("oops")

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c6", temporary=True)
        cmd.cancel(raise_error=False)   # must not raise

        self.assertIsNone(cmd.command_id)

    @patch("atexit.unregister")
    def test_cancel_propagates_when_raise_error_true(self, _):
        ctx = _make_ctx(context_id="ctx-c7", temporary=True)
        _ws(ctx.cluster).cancel.side_effect = RuntimeError("oops")

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c7", temporary=True)
    ...
    def test_unsafe_cancel_idempotent(self, _):
        ctx = _make_ctx(context_id="ctx-c8", temporary=True)
        _ws(ctx.cluster).cancel.return_value = MagicMock()

        cmd = _make_cmd(ctx=ctx, command_id="cmd-c8", temporary=True)
        cmd._unsafe_cancel()
        cmd._unsafe_cancel()   # second call: command_id is None → no-op
    ...
    def test_client_terminated_session_raised_on_keyword(self):
        ...
        resp = CommandStatusResponse(
            ...
            results=Results(
                ...
                cause="client terminated the session unexpectedly",
            ),
        )
...
class TestCleanupRemotePayload(unittest.TestCase):
    ...
    def test_no_payload_path_is_noop(self):
        ...
        cmd._cleanup_remote_payload()   # must not raise
...
class TestResultInner(unittest.TestCase):
    ...
    def _cmd_with_finished_details(self, data: str = "") -> CommandExecution:
        cmd = _make_cmd(command_id="cmd-ri", temporary=False)
    ...
    def test_result_inner_deserialises_tagged_payload(self, _):
        ...
        cmd = self._cmd_with_finished_details(data)
    ...
    def test_result_inner_retries_on_internal_error(self, mock_unreg, mock_reg):
        """_result_inner must reconnect and restart on InternalError, then succeed."""
        ctx = _make_ctx(context_id="ctx-ri2", temporary=True)
        ws = _ws(ctx.cluster)
        ws.execute.return_value = _exec_response("cmd-ri2-new")
        ws.create.return_value = MagicMock(response=MagicMock(id="ctx-ri2-new"))

        cmd = _make_cmd(ctx=ctx, command_id="cmd-ri2", temporary=True)
        ...
        def details_side_effect(self_cmd):
            ...
            # Subsequent calls return FINISHED so the retry loop exits cleanly
            return _finished("cmd-ri2-new", data="ok")

        with patch.object(
            CommandExecution, "details",
            new_callable=lambda: property(details_side_effect),
        ):
            result = cmd._result_inner(wait=False, raise_error=True)

        # Reconnect triggered a new execute call
        ws.execute.assert_called()
        # Second attempt succeeded → result returned
        self.assertEqual(result, "ok")

    @patch("atexit.unregister")
    def test_result_inner_raises_when_exhausted_and_raise_error_true(self, _):
        cmd = _make_cmd(command_id="cmd-ri3", temporary=False)
    ...
    def test_result_inner_no_raise_returns_raw_on_empty_output(self, _):
        """With raise_error=False and a FINISHED command with no tagged output,
        _result_inner must return the raw text (empty string) rather than raise."""
        cmd = _make_cmd(command_id="cmd-ri4", temporary=False)
        cmd._details = _finished("cmd-ri4", data="")  # FINISHED, no tag, empty data
...
class TestCommandExecutionWait(unittest.TestCase):
    ...
    def test_wait_on_finished_command_returns_self(self, _):
        cmd = _make_cmd(command_id="cmd-w1", temporary=False)
    ...
    def test_wait_on_no_command_id_calls_start_first(self, _):
        ctx = _make_ctx(context_id="ctx-w2", temporary=True)
        _ws(ctx.cluster).execute.return_value = _exec_response("cmd-w2")

        cmd = _make_cmd(ctx=ctx, command_id=None, temporary=True)
        # Seed _details as finished so the while-loop exits immediately
        with patch.object(type(cmd), "details", new_callable=lambda: property(
            lambda self: _finished("cmd-w2")
        )):
    ...
    def test_wait_raises_for_error_state_when_raise_error_true(self, _):
        cmd = _make_cmd(command_id="cmd-w3", temporary=False)
...
# ===========================================================================
# _zip_local_module() — static helper
# ===========================================================================

class TestZipLocalModule(unittest.TestCase):

    def test_zips_single_file(self, tmp_path=None):
        import tempfile
