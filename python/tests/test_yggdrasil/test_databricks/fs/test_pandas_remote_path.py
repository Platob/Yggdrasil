...
# ---------------------------------------------------------------------------
# Fake VolumePath — in-memory store that intercepts the SDK transport hooks
# ---------------------------------------------------------------------------


class _InMemoryVolumePath(VolumePath):
    ...
    machinery — URL handling, ``open_io`` mode dispatch, BytesIO
    transaction buffer, format-leaf dispatch by extension — runs
    unmodified. That's the surface we want under test.
    """
    ...
    def _remote_upload(self, payload) -> None:
        ...
        self._STORE[self.full_path()] = data
