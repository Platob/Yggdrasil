"""Tests for :class:`yggdrasil.io.base.IO` cursor surface.

:class:`IO` is the canonical cursor over a :class:`Holder`.
After the Holder / IO merge it inherits :class:`Holder` directly,
so every IO carries a :attr:`parent` (the byte substrate) and
...
Construction tests that only care about dispatch (no sequential
cursor ops) use ``IO(...)`` directly.
"""
...
def _cursor(data: bytes = b"", *, mode: str = "rb+") -> IO:
    """Create an IO cursor over a Memory parent seeded with *data*."""
    return Memory(data).open(mode)
...
class TestRead:
    ...
    def test_read_all(self) -> None:
        assert _cursor(b"abc").read() == b"abc"
...
class TestModes:
    ...
    def test_rb_does_not_truncate(self) -> None:
        mem = Memory(b"seed")
        with IO(holder=mem, owns_holder=False, mode="rb") as b:
            assert b.read() == b"seed"
        assert mem.read_bytes() == b"seed"

    def test_wb_truncates_on_open(self) -> None:
        mem = Memory(b"seed")
        with IO(holder=mem, owns_holder=False, mode="wb") as b:
            assert b.size == 0
            b.write(b"new")
    ...
    def test_ab_positions_at_end(self) -> None:
        mem = Memory(b"abc")
        with IO(holder=mem, owns_holder=False, mode="ab") as b:
            assert b.tell() == 3
            b.write(b"DEF")
    ...
    def test_xb_raises_on_non_empty(self) -> None:
        ...
        with pytest.raises(FileExistsError):
            with IO(holder=mem, owns_holder=False, mode="xb"):
                pass

    def test_xb_succeeds_on_empty(self) -> None:
        mem = Memory()
        with IO(holder=mem, owns_holder=False, mode="xb") as b:
            b.write(b"fresh")
...
class TestCursorParent:
    ...
    def test_multiple_cursors_over_one_parent(self) -> None:
        mem = Memory(b"shared")
        a = IO(holder=mem, owns_holder=False, mode="rb")
        b = IO(holder=mem, owns_holder=False, mode="rb")
        a.seek(2)
