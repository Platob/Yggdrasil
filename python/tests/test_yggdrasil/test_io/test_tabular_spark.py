...
class TestSparkTabularNativeWriteRouting(unittest.TestCase):
    """:meth:`Tabular._write_table` skips the Arrow round-trip when the
    source advertises a native Spark frame via
    :meth:`Tabular._native_spark_frame`.

    Mirrors the routing the real :class:`databricks.table.Table` sink
    relies on: a :class:`SparkTabular` source feeds straight into
    :meth:`_write_spark_frame` (and from there into ``spark_insert``
    on the table) instead of collecting to the driver first.
    """

    def test_native_spark_frame_default_is_none(self) -> None:
        from yggdrasil.io.tabular import SparkTabular

        empty = SparkTabular()
        self.assertIsNone(empty._native_spark_frame())

    def test_native_spark_frame_returns_held_frame(self) -> None:
        from yggdrasil.io.tabular import SparkTabular

        df = _fake_frame()
        io = SparkTabular(df)
        self.assertIs(io._native_spark_frame(), df)

    def test_write_table_routes_dataset_via_write_spark_frame(self) -> None:
        from yggdrasil.io.tabular import SparkTabular

        df = _fake_frame()
        source = SparkTabular(df)

        # Sink is another SparkTabular — both `_write_spark_frame` and
        # `_write_arrow_batches` are real methods we can spy on.
        sink = SparkTabular()
        with mock.patch.object(sink, "_write_spark_frame") as write_spark, \
             mock.patch.object(sink, "_write_arrow_batches") as write_arrow:
            sink.write_table(source)

        write_spark.assert_called_once()
        called_frame, _opts = write_spark.call_args.args
        self.assertIs(called_frame, df)
        write_arrow.assert_not_called()

    def test_write_table_empty_dataset_falls_through_to_arrow(self) -> None:
        # ``frame=None`` → no native frame → the existing Arrow drain
        # path runs (a no-op for an empty source, but it must not
        # crash by handing ``None`` to ``_write_spark_frame``).
        from yggdrasil.io.tabular import SparkTabular

        source = SparkTabular()  # frame is None
        sink = SparkTabular()
        ...
        write_spark.assert_not_called()
        write_arrow.assert_called_once()
