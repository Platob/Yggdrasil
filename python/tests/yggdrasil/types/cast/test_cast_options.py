...
#   yggdrasil.data.cast.CastOptions
#
# Run:
#   pytest -q
...
def test_safe_init_sets_source_and_target_fields_from_arrow_objects():
    ...
    opt = CastOptions(source_field=src, target_field=tgt, safe=True, eager=True)
...
def test_check_arg_kwargs_trigger_copy_not_in_place():
    base = CastOptions(safe=False, target_field=pa.field("x", pa.int32()))
...
def test_copy_datetime_patterns_edge_cases():
    ...
    base = CastOptions(datetime_patterns=[])
    c = base.copy(datetime_patterns=["%Y-%m-%d"])
    assert c.datetime_patterns == ["%Y-%m-%d"]

    base2 = CastOptions(datetime_patterns=["%Y"])
...
def test_check_source_only_sets_when_unset_and_obj_not_none():
    opt = CastOptions(target_field=pa.field("x", pa.int32()))
...
def test_need_arrow_type_cast_and_nullability_check():
    ...
    opt_same = CastOptions(source_field=src, target_field=pa.field("a", pa.int32(), nullable=True))
    ...
    opt_diff = CastOptions(source_field=src, target_field=pa.field("a", pa.int64(), nullable=True))
    assert opt_diff.need_arrow_type_cast(source_obj=None) is True

    opt_null_tighten = CastOptions(source_field=src, target_field=pa.field("a", pa.int32(), nullable=False))
    assert opt_null_tighten.need_nullability_fill(source_obj=None) is True

    opt_null_ok = CastOptions(
...
def test_target_field_name_fallbacks():
    opt0 = CastOptions(target_field=None)
...
def test_target_arrow_schema_struct_unwrap_and_scalar_wrap():
    ...
    opt = CastOptions(target_field=tf)
    ...
    opt2 = CastOptions(target_field=pa.field("z", pa.int64()))
