"""Benchmark :class:`ArrowIPCIO` — IPC stream / file format.
...
def scenarios(repeat: int) -> list[dict]:
    out: list[dict] = []
    out.extend(bench_roundtrip("arrow_ipc flat 1k", ArrowIPCIO, flat_table(1_000),
                               repeat=repeat, inner=500))
    out.extend(bench_roundtrip("arrow_ipc flat 50k", ArrowIPCIO, flat_table(50_000),
                               repeat=repeat, inner=100))
    out.extend(bench_roundtrip("arrow_ipc nested 10k", ArrowIPCIO, nested_table(10_000),
                               repeat=repeat, inner=100))
    out.extend(bench_roundtrip("arrow_ipc wide 32x10k", ArrowIPCIO, wide_table(10_000),
                               repeat=repeat, inner=100))
    ...
    out.append(time_one(
        ...
        lambda: ArrowIPCIO(payload).collect_schema(),
        repeat=repeat, inner=2_000,
    ))
    out.append(time_one(
        "arrow_ipc: read_arrow_batches flat 50k @ batch=8k",
        lambda: list(ArrowIPCIO(payload).read_arrow_batches(
