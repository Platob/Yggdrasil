...
### The IO / Tabular / Path / Holder inheritance chain is load-bearing

`Holder → IO → Path → RemotePath → DatabricksPath → Table` (and similar chains for `VolumePath`, `WorkspacePath`, `DBFSPath`) is the project's unified abstraction for anything that holds data, exposes a URL identity, and plugs into the singleton / pickle / cache machinery. Every class in this chain earns its place: `Tabular` gives the Arrow batch hooks, `Holder`/`IO` give URL identity + hash + equality + scheme registration, `Path` gives filesystem semantics, `RemotePath` gives singleton caching + stat caching, `DatabricksPath` gives service binding + dispatcher routing. **Never remove a base class from an existing resource in this hierarchy.** If a class overrides every method from a parent with stubs / raises, that's the design working — the stubs document "this resource doesn't support that surface" while keeping the type hierarchy intact for `isinstance` checks, the dispatcher, pickle, and the `CacheConfig` / `SendConfig` / `FolderPath` machinery that routes on these types. Refactoring away a base class breaks downstream `isinstance` gates, `Tabular.from_()` dispatch, `Holder.from_()` dispatch, and the singleton identity chain in ways that only surface at runtime in integration tests. **Fix the call site, not the hierarchy.**
