...
- **Cleanup-previous-release convention.** `publish.yml` removes the previous release when either (a) any commit on the push carries the literal token `--rm-previous-release` (opt-in, cross-minor — for shipping a broken release off the index), or (b) the new version is a patch bump of the previous tag (same `MAJOR.MINOR`, e.g. `1.2.3 → 1.2.4`; `1.2.5 → 1.3.0` does not trigger it). The previous tag is the highest semver `v*` tag below the new version; the job deletes its git tag + GitHub Release after the new release lands. PyPI yanking is *not* automated (no public API): the workflow summary surfaces the `pypi.org/manage/...` link so the maintainer can yank in one click.
