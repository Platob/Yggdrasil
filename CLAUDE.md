...
- **`raw_<entity>` carries provenance.** Add `_ingested_at: timestamp("UTC")`, `_source: string`, `_source_url: string`, `_payload_hash: string` (xxhash64), `_batch_id: string` as standard columns on every raw landing. Idempotency keys are `(<source_natural_id>, _ingested_at)`. `yggdrasil.io.response.RESPONSE_SCHEMA` is the canonical shape when the source is an HTTP `SchemaSession` cache.
...
- **Geographic data is always renderable.** Any curated table with a location reference carries `lat: float64 [-90, 90]` + `lon: float64 [-180, 180]` (WGS84) so frontends and map plugins (kepler.gl, pydeck, folium, Databricks SQL geo visual) can plot rows without a second lookup. Zone / region shapes also carry `boundary_geojson: string` (a GeoJSON `Feature` / `FeatureCollection`); tracks / time-series of positions use `points: list<struct<lat, lon, observation_utc>>`. Resolve at curate time via `yggdrasil.data.enums.geozone.GeoZoneCatalog` (every `GeoZone` already carries `lat` / `lon` / `country_iso` / `eic`); the `<catalog>.iso.*` shared dims (`country`, `region`, `exchange`, `bidding_zone`, …) all ship the geo columns so FK joins inherit them for free. Don't store WGS84 coords as `decimal` — geo libraries expect `float64`.

## HTTP ingestion: cache tier + persistent-failure notification

- **Pick the cache tier deliberately.** `HTTPSession` covers the basic path; `yggdrasil.databricks.schema.session.SchemaSession` adds per-URL Delta caching with `(mode, local_cache)` tuning. Decision tree: static reference data → `local=True` + `Mode.APPEND`; multi-worker Spark / `send_many` → `local=False` (workers can't see each other's local Arrow files, the remote Delta tier is the synchronization point); time-series / always-refetch → `local=False` + `Mode.UPSERT`; one-shot batch → plain `HTTPSession` + `Table.insert`. See `python/src/yggdrasil/databricks/schema/session.py`.
- **Persistent failures must not break the pipeline.** `_TieredRetry` (`python/src/yggdrasil/io/http_/session.py`) already gives 429 / 5xx a status-aware backoff and respects `Retry-After`. For failures that outlast the retry budget, use `ErrorNotifyingHTTPSession` (`yggdrasil.io.http_.notifying_session.ErrorNotifyingHTTPSession`) — it forces `raise_error=False` on the inner config, returns the failing response to the caller, and fires an `on_failure(response, exc, session)` notifier (use `smtp_email_notifier(...)` for stdlib email alerts, or any callable for Slack / PagerDuty). Set `raise_on_failure=True` for the strict shape. Notifier exceptions are caught + logged — never load-bearing.
