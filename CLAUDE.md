...
| `/api/v2/analysis` | polars **lazy**-over-Arrow analytics (scan + projection/predicate/slice pushdown + streaming) — `/aggregate`, `/describe`, `/finance` (price series → returns, cum-return, rolling mean/vol, EMA, drawdown + a scalar risk summary: total return, CAGR, annualized return/vol, Sharpe, Sortino, max drawdown, Calmar), `/series` (adaptive downsample + x-zoom), `/ohlc` (candlesticks). All take `filters` (predicate pushdown). `/export` applies a `Transform` (filters + casts incl. timezone→UTC + projection) and downloads in any media type (csv/parquet/json/ndjson/arrow/xlsx). `?node=` proxied. Drives the `TabularModal` Analyze panel (pivot/series/candles + collapsible filters&casts + Download-as) and `Chart` (bar/line/area/candle, MA overlay, volume panel). Analyze fetches are client-cached |
| `/api/v2/saga` | **Saga** distributed catalog — `catalog`/`schema`/`table` CRUD (static technical ids; column + statistics metadata, `/refresh`, `/discover`, per-table `/log` + `/activity`, `/search`) + SQL editor: `/sql` (JSON grid, node-failover), `/sql.arrow` (Arrow IPC, **batch-streamed** so peak memory ≈ one chunk), `/sql.materialize` (→ tmp parquet path for the shared /tabular+/analysis), `/sql.stage`, `/sql.export` (any media type), `/explain`, `/plan` (DAG + analyze + edit). Replication: `/replicate` (metadata\|data → peer) + `/import`. **Mounts** — `/mount` CRUD (named alias → a base path/URL: Databricks volume, S3 prefix, `npfs://` node, node-rooted folder, **or a live-database connection URI** — `postgres://`/`mysql://`/`sqlite://`/`mssql://`…, kind `database`, read via connectorx, bounded by `saga_db_row_limit`) + `/mount/{alias}/ls` (lazy browse through the `Path` layer — or `information_schema` for a DB mount — tabular children/tables flagged). A mount expands everywhere a source resolves — `SELECT * FROM 'alias/sub.parquet'` (file) or `'pgmount/schema.table'` (DB), a table's `source_url`, `/discover` + `/refresh` — and shows as a root in the `/fs` tree. Executes via `plan.execute` over registered tables, views (definition substituted), mounts, and node-rooted file URLs (incl. zip) with pushdown. Managed store in `~/.saga/{node_id}`. `?node=` proxied; **compute follows the data *and* resources** — a busy node offloads a query to a freer peer that holds a data replica |
